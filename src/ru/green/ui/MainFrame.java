...
public class MainFrame extends JFrame {
    ...
    public MainFrame() {
        ...
        runButton.setIcon(Utils.getIcon(DBAnalyzer.getProperty("run-button.icon")));
        ...
        reportButton.setToolTipText(DBAnalyzer.getProperty("report-query.tooltip"));
        reportButton.setIcon(Utils.getIcon(DBAnalyzer.getProperty("report-query.icon")));
        ...
        bookmarkButton.setToolTipText(DBAnalyzer.getProperty("bookmark.tooltip"));
        bookmarkButton.setIcon(Utils.getIcon(DBAnalyzer.getProperty("bookmark.icon")));
        ...
        saveButton.setIcon(Utils.getIcon(DBAnalyzer.getProperty("save-query.icon")));
        ...
        newButton.setIcon(Utils.getIcon(DBAnalyzer.getProperty("new-query.icon")));
        ...
        historyButton.setIcon(Utils.getIcon(DBAnalyzer.getProperty("history.icon")));
        ...
        toolbar.getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW).put(KeyStroke.getKeyStroke(KeyEvent.VK_E,
                InputEvent.CTRL_DOWN_MASK), "executeQuery");
        toolbar.getActionMap().put("executeQuery", executeQueryAction);
        toolbar.getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW).put(KeyStroke.getKeyStroke(KeyEvent.VK_B,
                InputEvent.CTRL_DOWN_MASK), "saveBookmark");

        toolbar.getActionMap().put("saveBookmark", saveBookmarkAction);
        toolbar.getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW).put(KeyStroke.getKeyStroke(KeyEvent.VK_S,
                InputEvent.CTRL_DOWN_MASK), "saveAction");
        toolbar.getActionMap().put("saveAction", saveAction);

        toolbar.getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW).put(KeyStroke.getKeyStroke(KeyEvent.VK_R,
                InputEvent.CTRL_DOWN_MASK), "report");
        toolbar.getActionMap().put("report", reportAction);

        toolbar.getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW).put(KeyStroke.getKeyStroke(KeyEvent.VK_N,
    ...
    public final void setTrayVisible(boolean b) {
        if (b) {
...
//		String sql = trimCommentsFromLeft(textArea.getText().trim());
    ...
    public void setResult(String[] columnNames, ArrayList<ArrayList> rows) {
        ...
        if (stPanel.getTextCount() != 0) {
            ...
            ToolWindow logTool = toolWindowManager.getToolWindow(DBAnalyzer.getProperty("tool-window.log.label"));
...
//		}
    ...
    private void initPopUpMenu() {
        ...
        rowSumm.setIcon(Utils.getIcon(DBAnalyzer.getProperty("result-tablel.sum.icon")));

        JMenuItem print = new JMenuItem(
                DBAnalyzer.getProperty("result-tablel.print"));
        print.addActionListener(printAction);
        KeyStroke ctrlP = KeyStroke.getKeyStroke(DBAnalyzer.getProperty("result-tablel.print.keystroke"));
        print.setAccelerator(ctrlP);
        print.setIcon(Utils.getIcon(DBAnalyzer.getProperty("result-tablel.print.icon")));

        JMenuItem report = new JMenuItem(
                DBAnalyzer.getProperty("result-tablel.report"));
        KeyStroke ctrlR = KeyStroke.getKeyStroke(DBAnalyzer.getProperty("result-tablel.report.keystroke"));
        report.setAccelerator(ctrlR);

        report.setIcon(Utils.getIcon(DBAnalyzer.getProperty("result-tablel.report.icon")));
    ...
    private void initToolWindowManager() {
        ...
        toolWindowManager.registerToolWindow(DBAnalyzer.getProperty("tool-window.bookmark.label"), // Id
                "", // Title
                (Utils.getIcon(DBAnalyzer.getProperty("tool-window.bookmark.icon"))), // Icon
                bookmarkPanel, // Component
                ToolWindowAnchor.LEFT); // Anchor

        toolWindowManager.registerToolWindow(DBAnalyzer.getProperty("tool-window.database.label"), // Id
                "", // Title
                Utils.getIcon(DBAnalyzer.getProperty("tool-window.database.icon")), // Icon
                dbPanel, // Component
                ToolWindowAnchor.LEFT); // Anchor

        toolWindowManager.registerToolWindow(DBAnalyzer.getProperty("tool-window.param.label"), // Id
                "", // Title
                Utils.getIcon(DBAnalyzer.getProperty("tool-window.param.icon")), // Icon
...
//        }
    ...
    public void setText(File file) {
        ...
        ToolWindow paramTool = toolWindowManager.getToolWindow(DBAnalyzer.getProperty("tool-window.param.label"));
    ...
    private JMenuBar createMenus() {
        ...
        JMenu fileMenu = menuBar.add(new JMenu(DBAnalyzer.getProperty("menu.file")));
        ...
        JMenu viewMenu = menuBar.add(new JMenu(DBAnalyzer.getProperty("menu.view")));
        ...
        JMenu serviceMenu = menuBar.add(new JMenu(DBAnalyzer.getProperty("menu.tools")));
        ...
        JMenu helpMenu = menuBar.add(new JMenu(DBAnalyzer.getProperty("menu.help")));
    ...
    private JMenuItem createMenuItem(JMenu menu, String label, String mnemonic,
        ...
        mi.setAccelerator(KeyStroke.getKeyStroke(DBAnalyzer.getProperty((keyStroke))));
    ...
    private JMenuItem createButtonGroupMenuItem(JMenu menu, String label,
        ...
        JRadioButtonMenuItem mi = (JRadioButtonMenuItem) menu.add(new JRadioButtonMenuItem(DBAnalyzer.getProperty(label)));
        buttonGroup.add(mi);
        if (DBAnalyzer.getProperty(mnemonic) != null
                && !DBAnalyzer.getProperty(mnemonic).isEmpty()) {
            mi.setMnemonic(getMnemonic(DBAnalyzer.getProperty(mnemonic)));
    ...
    private JMenuItem createCheckBoxMenuItem(JMenu menu, String label,
        ...
        JCheckBoxMenuItem mi = (JCheckBoxMenuItem) menu.add(new JCheckBoxMenuItem(DBAnalyzer.getProperty(label)));
        if (DBAnalyzer.getProperty(mnemonic) != null
                && !DBAnalyzer.getProperty(mnemonic).isEmpty()) {
            mi.setMnemonic(getMnemonic(DBAnalyzer.getProperty(mnemonic)));
        }
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            for (int row : rows) {
                for (int cell = 0; cell < columnCount; cell++) {
                    text.append(String.valueOf(resultTable.getValueAt(row, cell)));
    ...
    class CountSelectedRowsAction extends AbstractAction {
        ...
        @Override
        public void actionPerformed(ActionEvent e) {
            Messager.resultMessage(DBAnalyzer.getProperty("msg.table.selected-rows")
                    + " "
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            try {
                ...
                for (int i = 0; i < resultTable.getRowCount(); i++) {
                    sum += Double.parseDouble(String.valueOf(resultTable.getValueAt(i, selectedColumn)));
                }
                BigDecimal bd = BigDecimal.valueOf(sum);
                if ((bd.doubleValue() - bd.intValue()) == 0) {
                    Messager.resultMessage(DBAnalyzer.getProperty("msg.table.sum") + " " + bd.intValue());
                } else {
                    Messager.resultMessage(DBAnalyzer.getProperty("msg.table.sum")
                            + " "
                            + Utils.formatDecimal(bd));// .setScale(2,
                }														// BigDecimal.ROUND_HALF_UP).doubleValue()
            } catch (NumberFormatException ex) {
                Messager.resultMessage(DBAnalyzer.getProperty("msg.table.format"));
            }
        }
    }
